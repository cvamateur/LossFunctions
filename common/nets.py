...
class NormScaleLayer(nn.Module):
    """
    ...
    def __init__(self, alpha: float = 1.0, train_alpha: bool = False, check_lower: bool = False):
        super(NormScaleLayer, self).__init__()
        if train_alpha:
            ...
            if check_lower:
                ...
                if alpha < alpha_low:
                    raise RuntimeError(f"Alpha must be at least: {alpha_low:.1f}")
