...
class TripletLoss(nn.Module):
    ...
    squared (bool):
        The Euclidean distances are squared.

    NOTE:
        Selecting the hardest negatives can in practice lead to bad local minima early on in training.
    """

    def __init__(self, margin: float, strategy: str = "batch_hard", squared: bool = True):
    ...
    def forward(self, feats, labels):
        return self._forward_impl(feats, labels)

    # ---------------------------- Batch All -------------------------------
    def _forward_batch_all(self, feats, labels):
        dist_mat = _calc_distance_matrix(feats, self.squared)
...
def _calc_distance_matrix(embeddings, squared: bool = False):
