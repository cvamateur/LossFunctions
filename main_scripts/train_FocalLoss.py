...
from losses.mining import FocalLoss
from common.nets import MNIST_Net
from common.cli_parser import get_focal_loss_args
...
def main(args):
    ...
    # Loss Function
    #################
    criterion = FocalLoss(args.weights, args.exponent).to(device)
    ...
    # Visualizer
    ################
    visualizer = FeatureVisualizer("FocalLoss", len(ds_train), len(ds_valid), args.batch_size,
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        visualizer.save_fig(epoch, alpha=args.weights, gamma=args.exponent, dpi=args.dpi)
        schedular.step()
